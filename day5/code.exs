defmodule Day5 do
    ...
    def jump(list, offset, inc) do

        if offset >= length(list) do
            inc
        else
            jump = list |> Enum.at(offset)
            nextlist = list 
                |> Enum.with_index 
                # |> IO.inspect
                |> Enum.map(fn({v, k}) -> if k == offset do v + 1 else v end end)
            ...
            jump(nextlist, offset + jump, inc + 1)
        end
    end
...
Day5.jump(directions, directions |> Enum.at(0), 0)
    |> IO.inspect
